...
test('JavaAtlas ships exactly 150 real articles with ja/en locale parity', async () => {
  for (let i = 1; i <= 6; i += 1) assert.ok(CONTENT_CATEGORIES.includes(`java-expansion-${String(i).padStart(3, '0')}.json`));
  const ids = new Set();
  let count = 0;
...
test('the shipped Java corpus can be localized at runtime in ja and en', async () => {
  ...
  assert.equal(ja.articles.length, 150);
  assert.equal(en.articles.length, 150);
