...
test('JavaAtlas ships exactly 340 real articles with ja/en locale parity', async () => {
  for (let i = 1; i <= 25; i += 1) assert.ok(CONTENT_CATEGORIES.includes(`java-expansion-${String(i).padStart(3, '0')}.json`));
...
test('the shipped Java corpus can be localized at runtime in ja and en', async () => {
  ...
  assert.equal(ja.articles.length, 340); assert.equal(en.articles.length, 340); assert.equal(ja.articles[0].requestedLocale, 'ja'); assert.equal(en.articles[0].requestedLocale, 'en'); assert.ok(ja.articles.every(article => article.why && article.tips)); assert.ok(en.articles.every(article => article.why && article.tips));
});
